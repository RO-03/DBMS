...
# ... (this route is unchanged from the last version) ...
    ...
    if request.method == 'POST':
        ...
        elif form_name == 'vendor_buy_form':
            vendor_id_str = request.form.get('vendor_id_buy') 
            product_id_str = request.form.get('product_id_buy')
            quantity_str = request.form.get('quantity_buy')
            unit_price_str = request.form.get('unit_price_buy')

            if not all([vendor_id_str, product_id_str, quantity_str, unit_price_str]):
                flash("Please fill in all fields for the purchase order.", 'error')
                return redirect(url_for('vendors_page'))
            try:
                vendor_id = int(vendor_id_str)
                ...
                flash("Invalid input: IDs and Quantity must be integers, Price must be a number.", 'error')
                return redirect(url_for('vendors_page'))

            try:
                cursor = conn.cursor()
                cursor.callproc('sp_PlaceVendorPurchase', [vendor_id, product_id, quantity, unit_price])
                conn.commit() 
                flash(f"Successfully purchased {quantity} of Product {product_id} from Vendor {vendor_id}.", 'success')
                cursor.close()
            except mysql.connector.Error as err:
                return handle_sql_error(err, "", url_for('vendors_page'))
...
# ... (this route is unchanged from the last version) ...
    ...
    if request.method == 'POST':
        ...
        elif form_name == 'consumer_buy_form':
            customer_id_str = request.form.get('customer_id')
            product_id_str = request.form.get('product_id')
            quantity_str = request.form.get('quantity')
            
            if not all([customer_id_str, product_id_str, quantity_str]):
                flash("Please fill in all fields for the sales order.", 'error')
                return redirect(url_for('consumers_page'))
            try:
                ...
                flash("Invalid input: All IDs and Quantity must be integers.", 'error')
                return redirect(url_for('consumers_page'))
            
            try:
                cursor = conn.cursor()
                cursor.callproc('sp_PlaceConsumerSale', [customer_id, product_id, quantity])
                conn.commit() 
                flash(f"Successfully sold {quantity} of Product {product_id} to Customer {customer_id}.", 'success')
                cursor.close()
            except mysql.connector.Error as err:
                return handle_sql_error(err, "", url_for('consumers_page'))
